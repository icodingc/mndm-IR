...
from reader_ops import data_reader_random2dssm as reader
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      custom_runner = reader.CustomRunner(batch_size=5,vp=image_preprocessing_fn)
    ...
    def clone_fn(batch_queue):
      ...
      elif FLAGS.dssm_model=='dssm_is':
        tf.logging.info('Importance sampling with batch %d' % FLAGS.batch_size)
        triplet_ops._dssm_is_loss(a,p,ns,FLAGS.gamma)
      else:
    ...
    with tf.Session(config=my_config) as sess:
        ...
        try:
            for step in xrange(160001):
                ...
                if step%50==0:
                    print('Step: ',step,"Time:",dural," Loss==",_loss)
                if step%20000==0:
                    #Save model
