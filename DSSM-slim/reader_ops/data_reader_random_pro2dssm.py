...
def data_iter():
    ...
    while True:
        ...
        for batch_idx in xrange(0,len(shuf_lists)):
            ...
            negs = np.random.choice(item_lsts,5,p=Item_sim_matrix[item2idx[cur_fg]])
            while cur_fg in negs:negs = np.random.choice(item_lsts,5,p=Item_sim_matrix[item2idx[cur_fg]])
...
class CustomRunner(object):
    def __init__(self,batch_size=10,shuffle=True,vp):
