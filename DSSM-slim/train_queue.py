...
import colorama
def PP(str):
    return colorama.Fore.YELLOW+str+colorama.Style.RESET_ALL
#import logging
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      ...
      current_items=['image_a','image_p','image_n1','image_n2','image_n3','image_n4','image_n5']
      rst = provider.get(current_items)
      a = image_preprocessing_fn(rst[0], train_image_size, train_image_size,prefix='anchor')
      p = image_preprocessing_fn(rst[1], train_image_size, train_image_size,prefix='positive')
      # change num_negs
      idx = np.random.choice(5,FLAGS.num_negs)
      feature=[a,p]
      for i in xrange(FLAGS.num_negs):
          n1=image_preprocessing_fn(rst[2+idx[i]], train_image_size, train_image_size,prefix='neg'+str(i))
          feature.append(n1)
      feature_ = tf.train.batch(
            feature,
            batch_size=FLAGS.batch_size,
            num_threads=FLAGS.num_preprocessing_threads,
            capacity=5 * FLAGS.batch_size)
      images = tf.concat(0,feature_)
    ...
    def clone_fn(batch_queue):
      ...
      rst = tf.split(0,FLAGS.num_negs+2,embedding)
      a,p,ns = rst[0],rst[1],rst[2:]
      if FLAGS.dssm_model=='svm':
        tf.logging.info('Traing Triplet with batch %d' % FLAGS.batch_size)
        triplet_ops._triplet_multi_loss(a,p,ns,alpha=FLAGS.alpha)
      else:
    ...
    #TODO add gradient_multipliers,get name from clones_gradients
    ...
    tf.logging.debug(PP('Trainable variables..'))
    for grad,var in clones_gradients:
        if grad is not None:
            summaries.add(tf.histogram_summary(var.op.name+'/gradients',grad))
            tf.logging.info(var.op.name)
    tf.logging.debug(PP('grad_multipliers..'))
