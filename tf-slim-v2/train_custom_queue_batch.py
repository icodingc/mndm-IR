...
from reader_ops import cars_reader_random_batch as reader
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      custom_runner = reader.CustomRunner(batch_size=16,vp=image_preprocessing_fn)
    ...
    def clone_fn(batch_queue):
      ...
      if FLAGS.dssm_model=='dssm':
        tf.logging.info('DSSM with batch just like magnet')
        triplet_ops._dssm_loss_batch(embedding,FLAGS.gamma)
      else:
...
#    clones_gradients = slim.learning.multiply_gradients(clones_gradients,gradient_multipliers)
    ...
    pre_train="log/cars_log/cars_random/model.ckpt-50000"
    ...
    with tf.Session(config=my_config) as sess:
        ...
        try:
            for step in xrange(60001):
                ...
                if step%100==0:
                    print(datetime.now(),'at step: ',step,"Time:%.2f"%dural," Loss=%.3f"%_loss)
                if step%20000==0:
                    saver.save(sess,FLAGS.train_dir+'/model.cars_batch',global_step=step)
        except Exception,e:
