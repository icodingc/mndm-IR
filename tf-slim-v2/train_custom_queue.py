...
from reader_ops import reader_pro as reader
...
def main(_):
  ...
  with tf.Graph().as_default():
    ...
    with tf.device(deploy_config.inputs_device()):
      custom_runner = reader.CustomRunner(batch_size=8,vp=image_preprocessing_fn)
    ...
    def clone_fn(batch_queue):
      ...
      rst = tf.split(0,8,embedding)
      a,p,ns = rst[0],rst[1],rst[2:]
      if FLAGS.dssm_model=='dssm':
        tf.logging.info('DSSM with batch loss')
        triplet_ops._dssm_loss_batch(a,p,ns,FLAGS.gamma)
      else:
    ...
    with tf.Session(config=my_config) as sess:
        ...
        try:
            for step in xrange(20001):
                ...
                if step%20000==0:
                    saver.save(sess,FLAGS.train_dir+'/model.ft',global_step=step)
        except Exception,e:
