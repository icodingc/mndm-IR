...
file_name = np.load('./reader_ops/matrix/name.npy').tolist()
file_clusters = np.load('./reader_ops/matrix/label.npy').tolist()
base_dir = '/home/zhangxuesen/data/'
#####train lst
namelst = [a.strip() for a in file_name]
## name2Item,item2set
vector = {}
for a in namelst:
    fg = a.split('/')[-2]
...
#############Get cluster
name2cluster = dict(zip(namelst,file_clusters))
cluster={}
for i,n in enumerate(file_clusters):
    if str(n) not in cluster:
        cluster[str(n)]=[]
    cluster[str(n)].append(namelst[i])
cluster_list = cluster.keys()
###############
###############
def data_iter():
    file_lists = namelst
    ...
    while True:
        ...
        for batch_idx in xrange(0,len(shuf_lists)):
            ...
            #TODO sampling from AP cluster
            #cur_cluster = name2cluster[img_a]
            negs = np.random.choice(cluster_list,5)
            while cur_fg in negs:negs = np.random.choice(cluster_list,5)

            a = open(base_dir+img_a,'r').read()
            p = open(base_dir+img_p,'r').read()
            n1 = open(base_dir+np.random.choice(cluster[negs[0]]),'r').read()
            n2 = open(base_dir+np.random.choice(cluster[negs[1]]),'r').read()
            n3 = open(base_dir+np.random.choice(cluster[negs[2]]),'r').read()
            n4 = open(base_dir+np.random.choice(cluster[negs[3]]),'r').read()
            n5 = open(base_dir+np.random.choice(cluster[negs[4]]),'r').read()
            yield a,p,n1,n2,n3,n4,n5
...
#        img_n6 = self.transfer(self.image_n6,'neg6')
    ...
    def thread_main(self, sess,coord):
        ...
        """
        for a,p,n1,n2,n3,n4,n5 in data_iter():
