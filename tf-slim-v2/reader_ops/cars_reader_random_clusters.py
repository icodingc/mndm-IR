...
##########clusters 
with open('./reader_ops/matrix/cluster_ap.pkl','r') as f:clusters = pickle.load(f)
item_cluster = sorted(clusters.keys())
cluster2idx = dict(zip(item_cluster,list(xrange(len(item_cluster)))))
name2cluster={}
for item in item_cluster:
    for img in clusters[item]:
        name2cluster[img.strip()] = item
sim_matrix = np.load('./reader_ops/matrix/sim_matrix_cars_ap.npy')
...
def data_iter():
    ...
    while True:
        ...
        for batch_idx in xrange(0,len(shuf_lists)):
            ...
            #Get name2cluster
            #Q = sim_matrix[cluster2idx[name2cluster[img_a]]]
            #Q4 = np.power(Q,4)
            #Q = Q4/np.sum(Q4)
            negs = np.random.choice(item_cluster,6)
            while solve(cur_fg,negs):
                negs = np.random.choice(item_cluster,6)
            a = open(base_dir+img_a,'r').read()
            p = open(base_dir+img_p,'r').read()
            n1 = open(base_dir+np.random.choice(clusters[negs[0]]),'r').read()
            n2 = open(base_dir+np.random.choice(clusters[negs[1]]),'r').read()
            n3 = open(base_dir+np.random.choice(clusters[negs[2]]),'r').read()
            n4 = open(base_dir+np.random.choice(clusters[negs[3]]),'r').read()
            n5 = open(base_dir+np.random.choice(clusters[negs[4]]),'r').read()
            n6 = open(base_dir+np.random.choice(clusters[negs[5]]),'r').read()
