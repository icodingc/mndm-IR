...
def evaluation(sess,
               ...
               summary_writer=None,
               global_step=None,           
               is_save_to_disk=False, #TODO add two params to save feature
               store_prefix=None,
               store_dir=None):
  ...
  if eval_op is not None:
    ...
    if is_save_to_disk:
        if not gfile.Exists(store_dir):gfile.MakeDirs(store_dir)
        filefeat_ap = []
        filefeat_ap2 = []
        filename_ = []
    else:
    ...
    for i in range(int(num_evals)):
      ...
      rst = sess.run(eval_op, eval_op_feed_dict)
      if is_save_to_disk:
        filefeat_ap.append(np.squeeze(rst[0]))
        filefeat_ap2.append(np.squeeze(rst[1]))
        filename_.append(rst[2])
    if is_save_to_disk:
      np.save(os.path.join(store_dir,store_prefix+'_feature_ap'),np.vstack(filefeat_ap))
      np.save(os.path.join(store_dir,store_prefix+'_feature_ap2'),np.vstack(filefeat_ap2))
      np.save(os.path.join(store_dir,store_prefix+'_filename'),np.hstack(filename_))
      print('Saved feature!!')
  ...
  if summary_op is not None:
    logging.info('Done not Execute summary op')
    pass
...
def evaluate_once(master,
  ...
  with sv.managed_session(
    ...
    final_op_value = evaluation(sess,
                                ...
                                is_save_to_disk=is_save_to_disk,
...
#  summary_writer = summary_io.SummaryWriter(logdir)
  ...
  while True:
    summary_writer = summary_io.SummaryWriter(logdir+'eval'+str(0))
